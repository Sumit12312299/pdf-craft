...
// 27. Compare PDFs (Generates Visual & Text Comparison Report)
export async function comparePdfs(bufferA, bufferB) {
  ...
  // Add Comparison Summary Cover Page
  const coverPage = resultPdf.addPage([595, 842]); // A4
  ...
  coverPage.drawText('PDF Visual Comparison Report', {
    x: 40,
    y: height - 60,
    size: 22,
    font: boldFont,
  ...
  coverPage.drawText('Generated by PDFCraft Enterprise Suite', {
    x: 40,
    y: height - 82,
    size: 11,
    font,
  ...
  coverPage.drawText('Document A (Original):', { x: 40, y: yPos, size: 14, font: boldFont, color: rgb(0.1, 0.1, 0.1) });
  coverPage.drawText(`Total Pages: ${countA}`, { x: 230, y: yPos, size: 14, font, color: rgb(0.3, 0.3, 0.3) });

  yPos -= 30;
  coverPage.drawText('Document B (Modified):', { x: 40, y: yPos, size: 14, font: boldFont, color: rgb(0.1, 0.1, 0.1) });
  coverPage.drawText(`Total Pages: ${countB}`, { x: 230, y: yPos, size: 14, font, color: rgb(0.3, 0.3, 0.3) });
  ...
  yPos -= 40;
  coverPage.drawText('Page Comparison Overview:', { x: 40, y: yPos, size: 16, font: boldFont, color: rgb(0.08, 0.12, 0.22) });

  for (let i = 0; i < maxPages && i < 20; i++) {
    yPos -= 25;
    if (yPos < 60) break;
    ...
    let statusText = 'Both Documents Present';
    let statusColor = rgb(0.1, 0.6, 0.3);

    if (hasA && !hasB) {
      statusText = 'Page Removed in Document B';
      statusColor = rgb(0.8, 0.2, 0.2);
    } else if (!hasA && hasB) {
      statusText = 'Page Added in Document B';
      statusColor = rgb(0.1, 0.5, 0.9);
    }

    coverPage.drawText(`Page ${i + 1}:`, { x: 50, y: yPos, size: 12, font: boldFont, color: rgb(0.2, 0.2, 0.2) });
    coverPage.drawText(statusText, { x: 140, y: yPos, size: 12, font, color: statusColor });
  }

  // Copy pages from both documents into report
  for (let i = 0; i < maxPages; i++) {
    ...
    if (i < countB) {
      const [pageB] = await resultPdf.copyPages(pdfB, [i]);
      resultPdf.addPage(pageB);
    }
  }

  resultPdf.setCreator('pdfCraft Visual Comparison Engine');
  resultPdf.setProducer('pdfCraft Engine');
